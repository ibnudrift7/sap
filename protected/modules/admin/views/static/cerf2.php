...
$this->pageHeader=array(
	...
	'subtitle'=>'FSSC 22000 Certification',
);
?>
	...
	<div class="span12">
		...
		<div class="widgetcontent">
			...
			<div class="divider5"></div>
				...
				<?php $type = 'cerfiso2_content' ?>
				<?php Common::createSetting($type, 'Content', 'text', 'y') ?>
				<?php foreach (Language::model()->getLanguage() as $key => $value): ?>
					<div class="pj-multilang-wrap myLanguage control-group" style="display: <?php if ($value->code==$this->setting['lang_deff']): ?>block<?php else: ?>none<?php endif ?>;" data-id="<?php echo $value->id ?>">
						<label class="control-label required" for="Setting_<?php echo $type ?>_<?php echo $value->code ?>"><?php echo $model[$type]['data']->label ?><span class="required"></span></label>
						<textarea id="Setting_<?php echo $type ?>_<?php echo $value->code ?>" name="Setting[<?php echo $type ?>][<?php echo $value->code ?>]" class="span11 redactor" rows="2"><?php echo $model[$type]['desc'][$value->code]->value ?></textarea>
				...
				<p class="help-block">NOTE: Picture landscape with min size 1000 x 2500px, Larger image will be automatically cropped.</p>
