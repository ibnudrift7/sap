...
ALTER TABLE `or_order`  ADD `grand_total` DECIMAL(13,2) NOT NULL  AFTER `is_read`,  ADD `promo_id` INT NOT NULL  AFTER `grand_total`,  ADD `promo_kode` VARCHAR(50) NOT NULL  AFTER `promo_id`,  ADD `discount_type` INT NOT NULL  AFTER `promo_kode`,  ADD `discount` DECIMAL(13,2) NOT NULL  AFTER `discount_type`,  ADD `discount_total` DECIMAL(13,2) NOT NULL  AFTER `discount`;

INSERT INTO `setting` (`id`, `name`, `label`, `value`, `type`, `hide`, `group`, `dual_language`, `sort`) VALUES
(NULL, 'invoice_date', 'Invoice Date', '', 'text', 0, 'data', 'n', 1),
(NULL, 'invoice_number', 'Invoice Number', '', 'text', 0, 'data', 'n', 1);
